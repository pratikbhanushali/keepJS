...
keep.get = function (params) {
	try {
		...
		result = keep.decrypt(result);

		if(typeof result === "object") {
			var tempVal = JSON.stringify(result);

			/* Check if the stringified value can be converted back to JSON */
			try {
				if(JSON.parse(tempVal)) {
					result = tempVal;
				}
			} catch (e) {
				// console.log(e);
...
keep.set = function(params) {
	try {
		...
		var encrypt = (params.encrypt 	!= undefined && params.encrypt  != "") ? params.encrypt : keep._defaults.encrypt;


		if(typeof value === "object") {
			var tempVal = JSON.stringify(value);
		...
		/* Store value in Cookie / localStorage / sessionStorage */
...
keep.encrypt = function(data) {
	try {

		///////////////////////////////////////////
		// AppendData is a random String that will
		// be appended to every encrypted data
		///////////////////////////////////////////

		var key = (typeof secKey !== "undefined" && secKey != "") ? secKey : "keepJS";
		var encryptedData = CryptoJS.AES.encrypt(data, key);

		var appendData = "*w!w&w#w";
		return encryptedData + appendData;
	} catch(e) {
		return data;
	}
}

keep.decrypt = function(data) {
	try {
		var appendData = "*w!w&w#w";
		...
		if(data.substr(data.length-8) === appendData) {
			
			/*code to decrypt*/
			var key = (typeof secKey !== "undefined" && secKey != "") ? secKey : "keepJS";
			var decryptedData = CryptoJS.AES.decrypt(data.substr(0, data.length-8), key);
...
keep.Error.report = function(params) {
